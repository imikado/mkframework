<?php
class module_moduleAuthWithInscriptionBootstrap{
	...
	public function _index(){
		...
		if(isset($tMessage['success']) and $tMessage['success']==1){
			...
			Cr&eacute;ation du fichier module/'.$sModuleName.'/view/inscription.php<br/>
			<br/>
	...
	private function generate(){
		...
		$sContent=module_builder::getTools()->stringReplaceIn(array(
										...
										'row_example' => $sRowName,
										'auth_login' => $sViewName, 
										'auth_inscription' => $sViewInscriptionName, 
										'loginField' => $sLoginField,
										'passField' => $sPassField,
										
									),
									'data/sources/fichiers/module/authwithinscriptionBootstrap/main.php'
		);
		...
		//login
		$sContent=module_builder::getTools()->stringReplaceIn(array(
										'exampleauth' => $sModuleName,
									),
									'data/sources/fichiers/module/authwithinscriptionBootstrap/view/login.php'
		);
		...
		$sContent=module_builder::getTools()->stringReplaceIn(array(
									...
									'data/sources/fichiers/module/authwithinscriptionBootstrap/view/inscription.php'
		);
		
		$oNewViewFile=new _file(_root::getConfigVar('path.generation')._root::getParam('id').'/module/'.$sModuleName.'/view/inscription.php');
		$oNewViewFile->setContent($sContent);
		$oNewViewFile->save();
