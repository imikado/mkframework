...
class VARmodel_userENDVAR extends abstract_model implements VARinterfaceModelENDVAR {
	...
	protected $sTable = 'VARtableENDVAR';
	protected $sConfig = 'VARprofilENDVAR';
	protected $tId = array('VARidFieldENDVAR');
	...
	public function findById($uId) {
		return $this->findOne('SELECT * FROM ' . $this->sTable . ' WHERE VARidFieldENDVAR=?', $uId);
	}
	...
	public function getSelect() {
		...
		if ($tab) {
			foreach ($tab as $oRow) {
				$tSelect[$oRow->VARidFieldENDVAR] = $oRow->VARloginFieldENDVAR;
			}
		}
		return $tSelect;
	}
	...
	public function getListAccount() {
		...
		if ($tAccount) {
			foreach ($tAccount as $oAccount) {
				...
				$tLoginPassAccount[$oAccount->VARloginFieldENDVAR][$oAccount->VARpasswordFieldENDVAR] = $oAccount;
			}
		}
