...
class #model_examplemodel# extends abstract_model{
	...
	public function findListByUser($user_id){
		return $this->findManySimple('#requeteFindListByUser#',$user_id);
	}
	...
	public function findListGroupByUser($user_id){
		...
		foreach($tRow as $oRow){
			$tGroup[$oRow->#exampleGroupUserGroupId#]=$oRow->#exampleGroupUserGroupId#;
		}
		return $tGroup;
	}
	public function updateUserGroup($user_id,$tGroup){
		$this->execute('DELETE FROM #exampleGroupUserTable# WHERE #exampleGroupUserUserId#=?',array($user_id));
		if($tGroup){
			foreach($tGroup as $group_id){
				$this->execute('INSERT INTO #exampleGroupUserTable# (#exampleGroupUserGroupId#,#exampleGroupUserUserId#) VALUES (?,?)',array($group_id,$user_id));
			}
		}
	}
